...
SDL_Surface** paths_to_surface(char** paths, size_t len) {
    ...
    SDL_Surface** res = malloc(sizeof(SDL_Surface*) * len);
    for(size_t i = 0; i < len; i++) {
        *(res + i) = path_to_surface(paths[i]);
...
SDL_Surface** scale_surfaces(SDL_Surface** surfaces, size_t len) {
    SDL_Surface** res = malloc(sizeof(SDL_Surface*) * len);
    for(size_t i = 0; i < len; i++) {
        SDL_Surface* curr = *(surfaces + i);
        *(res + i) = SDL_CreateRGBSurfaceWithFormat(0, 28, 28, curr->format->BitsPerPixel, curr->format->format);
        SDL_Rect rect = {0, 0, 28, 28};
        SDL_BlitScaled(curr, NULL, *(res + i), &rect);
...
void to_blacks_and_whites(SDL_Surface** surfaces, size_t len) {
    for(size_t i = 0; i < len; i++) {
        ...
        int pixelCount = (*(surfaces + i))->w + (*(surfaces + i))->h;
        for(int j = 0; j < pixelCount; j++) {
            ...
            Uint32 grayPixel = SDL_MapRGBA((*(surfaces + i))->format, gray, gray, gray, a);
            ((Uint32*)(*(surfaces + i))->pixels)[i] = grayPixel;
        }
...
matrix** imgs_to_matrix(SDL_Surface** surfaces, size_t len) {
    matrix** res = malloc(sizeof(matrix*) * len);
    for(size_t i = 0; i < len; i++) {
        Uint32* pixels = (Uint32*) ((*(surfaces + i))->pixels);
        int pixelCount = (*(surfaces + i))->w + (*(surfaces + i))->h;
